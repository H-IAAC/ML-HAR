...
def conv1d(name, param, adaptation, meta):
    w = nn.Parameter(torch.ones(param['out-channels'], param['in-channels'], param['kernal']))
    torch.nn.init.kaiming_normal_(w)
    b = nn.Parameter(torch.zeros(param['out-channels']))
    w.meta, b.meta = meta, meta
    w.adaptation, b.adaptation = adaptation, adaptation
...
def LSTM(param, adaptation, meta):
    print('LSTM 0')
    w = nn.Parameter(torch.ones(param['filters'], param['hidden'], param['layers']))
    print('LSTM 1')
    torch.nn.init.kaiming_normal_(w)
    b = nn.Parameter(torch.zeros(param['layers']))
    w.meta, b.meta = meta, meta
    w.adaptation, b.adaptation = adaptation, adaptation
    return w, b

def linear(name, out_dim, in_dim, adaptation, meta):
