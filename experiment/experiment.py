...
class experiment:
    ...
    def __init__(self, name, args, output_dir="../", commit_changes=False, rank=None, seed=None):
        ...
        if not args is None:
            ...
            folder = datetime.datetime.now().strftime("%d%B%Y") 

            if not os.path.exists(output_dir + self.name):
                try:
                    os.makedirs(output_dir + self.name)
                except:
                    assert (os.path.exists(output_dir + self.name))
         
            self.root_folder = output_dir + self.name

            full_path = self.root_folder + "/" + folder + self.id
            ver = 0
