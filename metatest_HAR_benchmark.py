...
from datasets.utils import assert_consistency, check_standardization_adapter_all_modes, standardization_pass_fail_all_modes, standardization_message,to_numpy_3d, concat_samples, standarize_data, check_standardization_adapter
from utils.utils import backward_transfer_t0,prepare_json_backwardTransfer_v2, backward_transfer_max, prepare_json_dataset, prepare_json_stats, prepare_json_forgetting,compute_forgetting, backward_transfer, average_stats_classes,average_stats
...
def train_stream(X, Y, device, maml, opt):
...
def main():
    ...
    #args['path'] = '/home/ic-unicamp/Meta2/abordagens/new_benchmark/encoders/oml/nc/pamap2/None/15September2025-25000/1_2/'
    ...
    mu  = np.asarray(obj.get('results').get('Standardization train - mu'))
    sd = np.asarray(obj.get('results').get('Standardization train - sd'))
    ...
    #my_experiment = experiment(args['name'], args, args['folder_id'] + args['dataset'] + "/"  , commit_changes=False, rank=args['runs_offline'], seed=1)
    ...
    X_train_raw_np = to_numpy_3d(data_train.X, channels_last=False).copy()
    X_test_raw_np  = to_numpy_3d(data_test.X,  channels_last=False).copy()

    train_std, test_std = standarize_data(train_dataset=data_train,
                                          test_dataset=data_test,
                                          mode="window_samplewise",
                                          #time_cut_ratio=0.80,
                                          mu = mu,
                                          sd = sd,
                                          apply_to_train=True)
    ...
    report = check_standardization_adapter_all_modes(
        ...
        mode="window_samplewise",
        #time_cut_ratio=0.80,
        eps=1e-8, tol_mean=0.5, tol_std=0.5
    )
    ok, msg = standardization_pass_fail_all_modes(report, tol_consistency=1e-4)
    
    equal_mu = np.array_equal(np.asarray(train_std.mu), mu)
    
    print('equal mu and mu_', equal_mu)
    ...
    print(msg)
    '''
    bad = report["sanity"]["test"]["out_of_tolerance_channels"]
    mu_te = report["sanity"]["test"].get("_mu_per_channel")
    sd_te = report["sanity"]["test"].get("_sd_per_channel")
    
    for name in bad:
        j = int(name[1:]) - 1   # <- 0-based index
        print(f"{name}: mu={mu_te[j]:.4f}, sd={sd_te[j]:.4f}")
    '''
    logger.info(" msg standardization new = %s", str(msg))
    logger.info(" ok standardization  new = %s", str(ok))  
    ...
    X_train_raw_np_base = to_numpy_3d(data_train_base.X, channels_last=False).copy()
    X_test_raw_np_base  = to_numpy_3d(data_test_base.X,  channels_last=False).copy()

    train_base_std, test_base_std = standarize_data(train_dataset=data_train_base,
                                          test_dataset=data_test_base,
                                          mode="window_samplewise",
                                          #time_cut_ratio=0.80,
                                          mu = mu,
                                          sd = sd,
    ...
    report = check_standardization_adapter_all_modes(
        X_train_raw=X_train_raw_np_base,
        X_train_z  =to_numpy_3d(train_base_std.X, channels_last=False),
        X_test_raw =X_test_raw_np_base,
        X_test_z   =to_numpy_3d(test_base_std.X,  channels_last=False),
        mu_train   =np.asarray(train_base_std.mu),   # keep original shapes (1,1,T)
        sd_train   =np.asarray(train_base_std.sd),
        mode="window_samplewise",
        #time_cut_ratio=0.80,
        eps=1e-8, tol_mean=0.5, tol_std=0.5
    )
    ...
    equal_mu = np.array_equal(np.asarray(train_base_std.mu), mu)
    ...
    logger.info(" msg standardization base = %s", str(msg))
    logger.info(" ok standardization  base = %s", str(ok))  
       
    # remove new classes from train and test samples for evaluating performance on base classes
    
    dataset = utils.remove_classes_ucihar(train_base_std, classes_base)
    ...
    dataset = utils.remove_classes_ucihar(test_base_std, classes_base)
    ...
    for total_classes in args['schedule']:
        ...
        #search for 'best' learning rate # REVER CONJUNTO DE AVALIAÇÃO
        ...
        for lr_search_runs in range(0, 5):
            ...
            dataset = utils.remove_classes_ucihar(train_std, classes_to_keep)
            ...
            for lr in lr_sweep_range:
                ...
                opt = torch.optim.Adam(maml.net.get_adaptation_parameters_meta_test(), lr=lr)
        ...
        for current_run in range(0, args['runs']):
            ...
            #print('\niterator_sorted', (iterator_sorted.dataset.Y))
            dataset = utils.remove_classes_ucihar(train_std, classes_to_keep)
            
            iterator_train = torch.utils.data.DataLoader(dataset, batch_size=args['batch_size'],
                                             shuffle=False, num_workers=1)
            #print('\niterator_train training', np.unique(iterator_train.dataset.Y))
            
            dataset = utils.remove_classes_ucihar(test_std, classes_to_keep)
            iterator_test = torch.utils.data.DataLoader(dataset, batch_size=args['batch_size'],
                                                        shuffle=False, num_workers=1)
                        
            #print('\niterator_test training', np.unique(iterator_test.dataset.Y))
            # remove classes base dataset 
